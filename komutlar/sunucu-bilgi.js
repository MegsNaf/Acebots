    ...
    var embed = new Discord.MessageEmbed()
      ...
      .addField(
        ...
        `**Sunucu Sahibi**: ${
          ...
          dogru[message.guild.verificationLevel]
        }\n**Premium Durumu:** \`${açıkmı ? "Aktif" : "Kapalı"}\``
      )
      .addField(
        "Diğer Bilgiler",
        `**Sunucu Oluşturulma Tarihi: ** ${moment(
          message.guild.createdAt
        ).format("DD")} ${
          aylar[moment(message.guild.createdAt).format("MM")]
        } ${moment(message.guild.createdAt).format(
          "YYYY"
        )}\n**Sunucuya Katılım Tarihi: ** ${moment(
          message.guild.joinedAt
        ).format("DD")} ${
          aylar[moment(message.guild.joinedAt).format("MM")]
        } ${moment(message.guild.joinedAt).format("YYYY")}\n**Kısaltması: ** ${
          message.guild.nameAcronym
        }\n**Varsayılan Kanal: ** ${
          message.guild.defaultChannel
            ? message.guild.defaultChannel
            : "Varsayılan Kanal Yok"
        }`
      )
      .addField(
        "Üye Bilgileri",
        `**250'den Fazla Üyesi var mı? ** ${
          message.guild.large
            ? ":white_check_mark:"
            : ":negative_squared_cross_mark:"
        }\n**Toplam Üye:** ${üyesayi}\n**Kullanıcılar: ** ${kullanıcılar} \n**Bot Sayısı:** ${botlar}\n**Banlı Sayısı:** ${await message.guild
          .fetchBans()
          .then(b => b.size)}`
      )
      .addField(
      ...
      .addField(
        "Roller",
        `**Rol Sayısı: ** ${(message.guild.roles.cache.map(a => a.name),
        message.guild.roles.cache.size)}`
      )
      .addField(
        "Kanallar",
        `**Kanal Sayısı: ** ${(message.guild.channels.cache.map(a => a.name),
        message.guild.channels.cache.size)}\n**AFK Kanalı: ** ${
          message.guild.afkChannel ? message.guild.afkChannel : "AFK Kanalı Yok"
        }\n**AFK Zaman Aşımı:** ${
          message.guild.afkTimeout ? message.guild.afkTimeout : "AFK Kanalı Yok"
        }`
      )
