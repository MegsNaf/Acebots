...
exports.run = async (client, message, args) => {
  function bulkDeletefunc(kaçyüzmesaj) {
    if (!kaçyüzmesaj)
      throw new TypeError("Vereceğiniz Mesaj Sayısı Belirtilmemiş!");
    let bölme = kaçyüzmesaj / 100;
    if (bölme % 1 !== 0)
      throw new TypeError("Vereceğiniz Mesaj Sayısı 100 Ve Katları Olmalıdır!");
  ...
  if (!message.guild.me.hasPermission("MANAGE_MESSAGES")) {
    const yetkimyokembed = new Discord.MessageEmbed()
      .setAuthor("❌ Yetersiz Yetki!")
      .setDescription(
        "**Bunu Yapabilmem İçin `Mesajları Yönet` Yetkisine Sahip Olmalıyım!**"
      )
      .setColor("RED");
  ...
  const hakkında = new Discord.MessageEmbed()
    .setTitle("Ace Bot Temizleme Sistemi")
    .setDescription(
      `>>> **Alttaki Seçeneklerden Kaç Adet Mesaj Silmek İstediğini Seç ve Keyfine Bak.**`
    );
