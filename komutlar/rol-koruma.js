...
exports.run = async (client, message, args) => {
  if (message.author.id !== "id")
    return message.channel.send("Bu Komut Bakımdadır");
  ...
  if (!kanal)
    return message.channel.send("Rol koruma log kanalını belirtmelisin");
  codare.set(`rolk_${message.guild.id}`, kanal.id);
  message.channel.send(
    "Rol koruma sistemi aktif edildi, log kanalı <#" +
      kanal +
      "> Olarak ayarlandı."
  );

  if (args[0] === "kapat") {
    if (!codare.fetch(`rolk_${message.guild.id}`))
      message.channel.send("Rol koruma sistemi aktif edilmemiş.");
